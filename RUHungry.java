...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            categoryVar[i] = category; // Reads category name

            int dishesCount = StdIn.readInt(); 
            ...
            for (int j = 0; j < dishesCount; j++) {
                ...
                int ingredientsCount = StdIn.readInt();
                int[] ingredientIDs = new int[ingredientsCount];

                for (int k = 0; k < ingredientsCount; k++) {
                    ingredientIDs[k] = StdIn.readInt(); // Reads each ingredient ID
                }

                Dish dish = new Dish(category, dishName, ingredientIDs);
                menuVar[i] = new MenuNode(dish, menuVar[i]); // Creates a new node and links it
            }
        }
    }
    ...
    public void updatePriceAndProfit() {
        ...
        for (int i = 0; i < menuVar.length; i++){
            ...
            while (ptr != null){
                ...
                for (int j = 0; j < dish.getStockID().length; j++ ){
                    StockNode targetStock = findStockNode(dish.getStockID()[j]);
                    stockCost += targetStock.getIngredient().getCost();
                }
                
                retailPrice = stockCost * 1.2;
                ptr.getDish().setPriceOfDish(retailPrice);
                ptr.getDish().setProfit(retailPrice - stockCost);
                ptr = ptr.getNextMenuNode();
            }
    ...
    public void createStockHashTable (String inputFile){
        ...
        StdIn.setFile(inputFile); // opens inputFile to be read by StdIn
         ...
         while (!StdIn.isEmpty()){
            ...
            StdIn.readChar();
            String ingredientString = StdIn.readLine();
            double price = StdIn.readDouble();
            int stockAmount = StdIn.readInt();
            Ingredient newIngredient = new Ingredient(ingredientID, ingredientString, stockAmount, price);
            StockNode node = new StockNode(newIngredient, null);
            addStockNode(node);
    ...
    public boolean checkDishAvailability (String dishName, int numberOfDishes){
        ...
        MenuNode DishinMenu = findDish(dishName);
        int[] ingredients = DishinMenu.getDish().getStockID();
        for (int i = 0; i < ingredients.length; i++){
            StockNode stock = findStockNode(ingredients[i]);
            if (stock.getIngredient().getStockLevel() == 0 || numberOfDishes > stock.getIngredient().getStockLevel()){
                return false;
            }
        }
    ...
    public void order (String dishName, int quantity){
        MenuNode dishMenuNode = findDish(dishName);
        String category = dishMenuNode.getDish().getCategory();
        
        double dishProfit = 0;
        if (checkDishAvailability(dishName, quantity)){
            dishProfit = quantity * findDish(dishName).getDish().getProfit();
            TransactionData orderData = new TransactionData("order", dishName, quantity, dishProfit, true);
            addTransactionNode(orderData);
            int[] stockID = dishMenuNode.getDish().getStockID();
            for (int i: stockID){
                updateStock(null, i, -quantity);
            }
            ...
            TransactionData failedData = new TransactionData("order", dishName, quantity, 0.00, false);
            addTransactionNode(failedData);

            MenuNode ptr = dishMenuNode.getNextMenuNode();
            boolean foundAvailable = false;
            MenuNode startNode = dishMenuNode;
            do {
                if (ptr == null) {
                    ptr = menuVar[findCategoryIndex(category)]; // Reset to start if null
                }
                dishProfit = quantity * ptr.getDish().getProfit();
                if (checkDishAvailability(ptr.getDish().getName(), quantity)) {
                    TransactionData orderData = new TransactionData("order", ptr.getDish().getName(), quantity, dishProfit, true);
                    addTransactionNode(orderData);
                ...
                failedData = new TransactionData("order", ptr.getDish().getName(), quantity, 0, false);
                addTransactionNode(failedData);
            }
                ptr = ptr.getNextMenuNode();
                if (ptr == null && !foundAvailable){
                    ptr = menuVar[findCategoryIndex(category)];
                }
            }while (!ptr.equals(startNode)  && !foundAvailable);
        }
    ...
    public void donation (String ingredientName, int quantity){
        ...
        if (profit() > 50 && findStockNode(ingredientName).getIngredient().getStockLevel() >= quantity){
            TransactionData donations = new TransactionData("donation", ingredientName, quantity, 0, true);
            addTransactionNode(donations);
            
            updateStock(ingredientName, -1, -quantity);
        }
        else{
            TransactionData donations = new TransactionData("donation", ingredientName, quantity, 0, false);
            addTransactionNode(donations);
        }
    ...
    public void restock (String ingredientName, int quantity){
	    ...
	    if (profit() >= restockCost){
            updateStock(ingredientName, -1, quantity);
            TransactionData successData = new TransactionData("restock", ingredientName, quantity, -restockCost, true);
            addTransactionNode(successData);
        }
        else{
            TransactionData failedData = new TransactionData("restock", ingredientName, quantity, 0, false);
            addTransactionNode(failedData);
        }
    }
